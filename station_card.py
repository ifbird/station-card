...
class MarkSite:
  ...
  def start(self):
    ...
    res = messagebox.askquestion( \
        'Mark sites', 'Do you want to mark new sites?' \
        )
...
card_root_dir = (
  f'/home/pzzhou/Documents/research/projects/'
  f'20220901_20250301-FOCI_EU_project/data_collection/station_cards'
  )
station = 'smearii_hyytiala'
station_dir = card_root_dir + '/' + station
card_json_file = 'smearii_hyytiala.json'
fpath_card_json = '{0}/{1}/{2}'.format(card_root_dir, station, card_json_file)
fpath_country_map = '{0}/country_map.png'.format(station_dir)
fpath_local_map = '{0}/smearii.png'.format(station_dir)
fpath_local_map_new = '{0}/local_map_new.png'.format(station_dir)
# fpath_card_map = '{0}/card_map.png'.format(station_dir)


#
# Create the card and prepare the figures
#

card = {}
card['smearii'] = StationCard(fpath_card_json)
# card['smearii'].print_card_layout_to_figure('{0}/card.png'.format(station_dir))

# Plot country map with global map inside
# print('Plotting country map ...')
# card['smearii'].prepare_country_map(fpath_country_map)

# Mark site locations and then save the figure
# print('Plotting local map and marking the sites ...')
# card['smearii'].prepare_local_map(fpath_local_map, fpath_local_map_new)
...
# Initiate
doc = CardDocument(card['smearii'].card_json, \
    documentclass='article', document_options=['a4paper', 'portrait'])

# Call function to add text
doc.fill_document(fpath_country_map, fpath_local_map_new)

# Generate the pdf file
doc.generate_pdf('{0}/card'.format(station_dir), clean_tex=False)
